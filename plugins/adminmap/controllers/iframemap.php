...
class Iframemap_Controller extends Template_Controller {
    ...
    {
	// HACK: different default co-ordinates/zoom level for wral page
	$wral_map = (isset($_SERVER['HTTP_REFERER']) && false !== strpos($_SERVER['HTTP_REFERER'], 'www.wral.com'));
	if ($wral_map) $width = 480;
	...
	plugin::add_javascript("adminmap/js/jquery.hovertip-1.0");
		...
		// HACK: different default co-ordinates/zoom level for wral page
		if ($wral_map) {
			$this->themes->js->default_zoom = 7;
			$this->themes->js->latitude = 35.128894;
			$this->themes->js->longitude = -78.299561;
		}
